...
app.get("/food", async (_req, res) => {
    ...
    messages: [
      {
        ...
        content: `You are a helpful assistant for ordering food on a restaurant.
          ...
          When the user confirms the order then your answer should contain a summary of the order,
          and at the end a token to indicate the end of the conversation, like this #order_confirmed#.`,
      },
      {
        role: "user",
        content:
          "Hi, what do you recommend for dinner? I'm in the mood for something spicy.",
      },
    ],
    stream: true,
  });
  ...
  // Send the AI's response to the client as streaming data
  for await (const chunk of openaiResponse) {
    const content = chunk.choices[0]?.delta?.content || "";
    buffer += content;

    if (buffer.includes(".")) {
      const currentBuffer = buffer.split(".")[0] + ".";
      buffer = buffer.split(".")[1];
      chunks.push(currentBuffer);

      console.log({ currentBuffer, buffer });
      res.write(currentBuffer);
    }
  }
...
app.post("/test", async (req, res) => {
  console.log("called test process_speech");

  // The transcribed text is in the 'SpeechResult' parameter
  const { body } = req;

  const speechResult = body.text;
  ...
  if (!conversation_contexts[phoneNumber]) {
    console.log("initialize conversation context");
  ...
  console.log("after init conversation_contexts:", conversation_contexts);
  ...
  console.log(
    "after Add the user's input to the conversation context:",
    conversation_contexts
  );

  console.log("before openaiResponse: ", conversation_contexts[phoneNumber]);
  ...
  console.log("after openaiResponse: ", conversation_contexts[phoneNumber]);

  // Respond to the user with the AI's response
  res.send(conversation_contexts[phoneNumber]);
});
...
app.post("/old_process_speech", async (req, res) => {
  console.log("called process_speech");
  const twiml = new VoiceResponse({ voice: "Polly.Matthew-Neural" });
    ...
    conversation_contexts[phoneNumber] = [
      {
        role: "system",
        content:
          "You are a helpful assistant for ordering food on a restaurant. You can take orders, answer questions, and provide recommendations. Make the response short and clear.",
      },
    ];
  }
